    ...
    <div className="bg-card border border-border rounded-xl p-6 flex flex-col hover:border-primary/50 hover:shadow-md transition-all h-full">
      ...
      <div className="flex-1 overflow-y-auto max-h-[400px] pr-2 space-y-2 custom-scrollbar">
        {activityLogs.map((activity) => {
          ...
          return (
            <div key={activity.id} className="flex items-start gap-4 p-3 rounded-lg hover:bg-muted transition-colors group">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1.5">
                  <span className={`text-[9px] font-bold uppercase px-2 py-0.5 rounded-full ${facilityColors['System' as keyof typeof facilityColors] || 'bg-muted text-muted-foreground'}`}>
                    System
                  </span>
