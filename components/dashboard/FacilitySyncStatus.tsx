...
export function FacilitySyncStatus() {
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-6 flex flex-col hover:border-primary/50 hover:shadow-md transition-all h-full">
      <h3 className="text-lg font-bold text-foreground tracking-tight mb-6">Facility Sync Status</h3>

      <div className="flex-1 space-y-4">
        {mockFacilities.map((facility) => (
          <div key={facility.id} className="pb-4 border-b border-border last:border-0">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex flex-col items-end">
                ...
                <span className="text-[10px] text-muted-foreground mt-1 font-medium">Last sync: {new Date(facility.lastSync).toLocaleDateString()}</span>
              </div>
            </div>
      ...
      <button 
        ...
        disabled={isSyncing}
        className="mt-6 w-full py-3 border-2 border-primary text-primary font-bold text-xs rounded-xl hover:bg-primary hover:text-primary-foreground transition-all flex items-center justify-center gap-2 disabled:opacity-50"
      >
